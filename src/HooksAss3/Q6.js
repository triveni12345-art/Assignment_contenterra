...
const Q6 = () => {
  ...
  return (
    <>
      ...
      <div>
        {count3}
        <button onClick={() => dispatch3({ type: 'increment' })}>+1</button>
        <button onClick={() => dispatch3({ type: 'decrement' })}>-1</button>
        <button onClick={() => dispatch3({ type: 'set', count: 0 })}>reset</button>
      </div>
    </>
  );
};
